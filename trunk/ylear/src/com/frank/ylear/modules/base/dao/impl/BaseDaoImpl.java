...
public class BaseDaoImpl extends HibernateDaoSupport implements BaseDao {
	...
	/*public SysUser getSysUserIncludeSysRight( Serializable id){
		...
		System.out.println("getSysUserIncludeSysRight开始");
		try
		{
			//取得Session
			session=getSession();
			...
			List<SysUser> ret = query.list();
			if(ret!=null&&ret.size()>0)
			{
				obj=ret.get(0);
				Set roleRights=obj.getSysRole().getSysRoleRights();
				Hibernate.initialize(roleRights);
				Iterator<SysRoleRight> it=roleRights.iterator();
				while(it.hasNext())
				{
					SysRoleRight roleRight=it.next();
					Hibernate.initialize(roleRight.getSysRight());
				}
			}
		}
		catch(Exception ex)
		...
		System.out.println("getSysUserIncludeSysRight结束");
		return obj;
	}*/
